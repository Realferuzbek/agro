...
export function IrrigationHistory() {const { duration, localDate, volume } = useFormat();const copy = useCopy();const locale=useLocale();
  const [runs,setRuns] = useState<Run[]>([]);const [error,setError]=useState('');const[loading,setLoading]=useState(true);
  const load=useCallback(async()=>{try{const response=await fetch('/api/history',{cache:'no-store'});const data=await response.json();if(!response.ok)throw new Error(data.error);setRuns(data.runs);setError('');}catch(e){setError(e instanceof Error?e.message:copy.irrigationHistory.historyIsUnavailable);}finally{setLoading(false);}},[copy.irrigationHistory.historyIsUnavailable]);
