...
export function GrowthTimeline({ stage }: { stage: string }) {
  ...
  return <div className="growth-stages">{labels.map((label, index) => <div key={label} className={`growth-stage ${index < active ? 'done' : index === active ? 'current' : ''}`}><div className="stage-line" /><strong>{label}</strong><span>{POTATO_PARAMETERS.demoStageDays[index]}{copy.growthTimeline.dayCountSuffix}</span></div>)}</div>;
}
...
export function CropCoefficientProgression({ daysAfterPlanting, currentKcb }: { daysAfterPlanting: number; currentKcb: number }) {
  ...
  return <div>
    ...
    <details style={{ marginTop: 16 }}><summary style={{ cursor: 'pointer', color: '#496442', fontSize: 12 }}>{copy.growthTimeline.compareTheFourGrowthStages}</summary><div className="table-wrap" style={{ marginTop: 8 }}><table><thead><tr><th>{copy.growthTimeline.stage}</th><th>{copy.growthTimeline.demoDays}</th><th>{copy.growthTimeline.referenceDays}</th><th>{copy.growthTimeline.kcbProgression}</th></tr></thead><tbody>{stageLabels.map((label, index) => <tr key={label}><td>{label}</td><td>{POTATO_PARAMETERS.demoStageDays[index]}</td><td>{POTATO_PARAMETERS.referenceStageDays[index]}</td><td>{coefficientLabels[index]}</td></tr>)}</tbody></table></div></details>
