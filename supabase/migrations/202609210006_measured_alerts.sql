...
create or replace function public.ingest_and_recalculate(
  ...
  p_expected_revision bigint default null,p_next_state jsonb default null,p_calculation jsonb default null,p_expected_latest_at timestamptz default null
) returns jsonb language plpgsql security invoker set search_path='' as $$
declare device public.devices; field_row public.fields; current_row public.product_state; receipt jsonb; calc_id uuid; next_revision bigint; previous_observed_at timestamptz; alert_item jsonb;
begin
  perform 1 from public.device_credentials where id=p_credential_id for update;
  select * into device from public.devices where id=p_device_id for update;
  select observed_at into previous_observed_at from public.device_latest_state where device_id=p_device_id;
  if p_next_state is not null then
    select * into field_row from public.fields where id=device.field_id for update;
  ...
  if p_next_state is not null and not (receipt->>'duplicate')::boolean then
    if previous_observed_at is distinct from p_expected_latest_at then raise exception 'Device baseline version conflict' using errcode='40001'; end if;
    ...
    if p_measurements ? 'rainfallIncrementMm' or p_measurements ? 'rainfallCumulativeMm' then
      insert into public.weather_observations(field_id,observed_at,source_id,provenance,data)
        values(device.field_id,p_observed_at,p_device_id,'MEASURED',p_measurements) on conflict do nothing;
    end if;
    for alert_item in select value from jsonb_array_elements(p_next_state->'alerts') where value->>'deviceId'=p_device_id loop
      insert into public.alerts(id,field_id,type,severity,detected_at,status,evidence)
        values(alert_item->>'id',device.field_id,alert_item->>'type',alert_item->>'severity',(alert_item->>'detectedAt')::timestamptz,alert_item->>'status',alert_item)
...
$$;
revoke all on function public.ingest_and_recalculate(uuid,text,text,text,timestamptz,text,jsonb,jsonb,bigint,jsonb,jsonb,timestamptz) from public,anon,authenticated;
grant execute on function public.ingest_and_recalculate(uuid,text,text,text,timestamptz,text,jsonb,jsonb,bigint,jsonb,jsonb,timestamptz) to service_role;
