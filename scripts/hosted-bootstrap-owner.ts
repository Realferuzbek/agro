...
async function main(){
  ...
  if(!user){
    const created=await service.auth.admin.createUser({email:designatedEmail,password:configuredPassword,email_confirm:true});if(created.error)throw created.error;user=created.data.user;
  }else{
    // Make the private bootstrap credential authoritative and repeatable without exposing it.
    const updated=await service.auth.admin.updateUserById(user.id,{password:configuredPassword,email_confirm:true});if(updated.error)throw updated.error;user=updated.data.user;
  }
