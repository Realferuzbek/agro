...
if(current.NEXT_PUBLIC_SUPABASE_URL && !['localhost','127.0.0.1','[::1]'].includes(new URL(current.NEXT_PUBLIC_SUPABASE_URL).hostname))throw new Error('Refusing to overwrite configuration for a hosted backend.');
...
const settings={...current,NEXT_PUBLIC_SUPABASE_URL:status.API_URL,NEXT_PUBLIC_SUPABASE_ANON_KEY:status.ANON_KEY,SUPABASE_SERVICE_ROLE_KEY:status.SERVICE_ROLE_KEY,NEXT_PUBLIC_SITE_URL:current.NEXT_PUBLIC_SITE_URL??'http://127.0.0.1:3000',AGRIFLOW_ADMIN_EMAIL:current.AGRIFLOW_ADMIN_EMAIL||'admin@agriflow.local',AGRIFLOW_ADMIN_PASSWORD:current.AGRIFLOW_ADMIN_PASSWORD||randomBytes(24).toString('base64url')};
